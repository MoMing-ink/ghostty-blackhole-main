    ...
    , m_rendererProcess(nullptr)
{
    ...
    // 升级兼容：清理旧版本的自启项（原名 BlakholeUI / BlackholeUI，已改名为 BlackHole）
    {
        QSettings oldReg("HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run",
                         QSettings::NativeFormat);
        oldReg.remove("BlakholeUI");
        oldReg.remove("BlackholeUI");
    }
...
void BlackHoleCore::saveConfig()
{
    ...
    if (m_autoStart) {
        ...
        appPath.replace('/', '\\');
        reg.setValue("BlackHole", appPath);
    } else {
        QSettings reg("HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run",
                      QSettings::NativeFormat);
        reg.remove("BlackHole");
    }
...
void BlackHoleCore::startRenderer()
{
    ...
    // 先保存配置到文件（blackhole_render.exe --render 从文件读取配置）
    saveConfig();

    // 查找 blackhole_render.exe（多级上溯，适配嵌套构建目录）
    QString appDir = QCoreApplication::applicationDirPath();
    QString exePath = appDir + "/blackhole_render.exe";

    if (!QFileInfo::exists(exePath)) {
        QStringList searchPaths;
        QDir dir(appDir);
        searchPaths << dir.absoluteFilePath("../blackhole_render.exe");
        searchPaths << dir.absoluteFilePath("../../blackhole_render.exe");
        searchPaths << dir.absoluteFilePath("../../../blackhole_render.exe");
        searchPaths << dir.absoluteFilePath("../../../build/blackhole_render.exe");
        searchPaths << dir.absoluteFilePath("../../../release/blackhole_render.exe");
    ...
    if (!QFileInfo::exists(exePath)) {
        QString msg = QStringLiteral("找不到 blackhole_render.exe");
        qWarning() << "BlackHoleCore:" << msg;
        emit rendererError(QStringLiteral("找不到 blackhole_render.exe，请确认程序位置"));
        return;
    }
